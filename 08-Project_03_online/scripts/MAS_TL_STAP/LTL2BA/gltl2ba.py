    ...
    def parse(ltl2ba_output, ignore_title=True):
        ...
        for line in ltl2ba_output.split('\n'):
            if Ltl2baParser.is_title(line):
                ...
                nodes[name] = [label]
                if accepting: 
                    nodes[name].append('accept')    # nodes的数据格式：{'name':[label,'accept'(若可接收则有这一项)]}
            elif Ltl2baParser.is_edge(line):
                dst_node, label = Ltl2baParser.get_edge(line)
                assert src_node is not None     # 满足这个条件时能继续跑下去，否则会报错AssertionError
                graph.edge(src_node, dst_node, label)
                edges[(src_node, dst_node)] = (label, parse_guard(label))   # edges的数据格式：{(src_node, dst_node):(label,parse_guard(label))}
...
def run_ltl2ba(ltl_formula, args=None, dev='win', mode='output'):   # 第一步 公式转换成包含ba信息的长字符串
...
def parse_promela(promela):     # 第二步：得到节点和边
