...
class Branch_And_Bound(object):
    ...
    def Begin_branch_search2(self, time_limit, up_bound_method, low_bound_method, shared_best, lock, search_method='DFS'):
        start = time.time()
        ...
        while (not self.branch_tree == []) and time.time() - start < time_limit:
            ...
            self.best_up_bound = shared_best['value']   # 读取当前最优值，以此来剪枝！！
            if tuple(task) in self.search_node_list.keys():
                ...
                if low_bound < self.best_up_bound:
                    ...
                    if up_bound < self.best_up_bound:
                        ...
                        self.best_up_bound = up_bound
                        self.best_up_bound_list[self.count_round] = (self.best_up_bound, time.time() - start)
                    with lock:      # 使用锁来确保进程安全,在这里更新全局最优解
                        if self.best_up_bound < shared_best['value']:
                            shared_best['value'] = self.best_up_bound
                            shared_best['solution'] = self.best_solution
                else:
                    self.low_bound_list[self.count_round] = (low_bound, time.time() - start, 'cut')
                    self.upper_bound_list[self.count_round] = (self.best_up_bound, time.time() - start)
    ...
    def found_solution_greedy(self, init_node, assigned_tasks, repeat_number=1):
        ...
        for i in assigned_tasks:
            req_score = 0
            for n in self.task_type[self.task_data[i][1]][1].values():
                req_score = req_score + n
            t2 = t2 + self.task_type[self.task_data[i][1]][0] * req_score/10     # t2不同
        ...
        while sequence != [] and un_found > 0:
            ...
            for to_assig_task in un_assig_task:     # un_assig_task: {0, 2, 4, 6}
                ...
                feasible = {}
                for sub_task, score in sub_task_list.items():
                    list1 = []   # 收集所有能执行这个sub_task的智能体id
                    for agent_i in self.agent_data:
                        if sub_task in self.agent_type[agent_i[2]]['serve'].keys():
                            list1.append(agent_i[0])
                    feasible[sub_task] = (list1, score)
                ...
                for z in range(repeat_number):
                    ...
                    while 1:
                        assign_list = {}
                        # check_list = []
                        for sub_task, (list1, score) in feasible.items():
                            agent_list = []
                            for i in range(3):  # 找三个智能体组合
                                agent_con_i = []
                                con_score = 0
                                while con_score < score:
                                    agent_id = random.choice(list1)
                                    if agent_id in agent_con_i:
                                        continue
                                    agent_con_i.append(agent_id)
                                    con_score = con_score + self.agent_type[self.agent_data[agent_id][2]]['serve'][sub_task]
                                agent_list.append(agent_con_i)
                            assign_list[sub_task] = agent_list
                        break
                    ...
                    for sub_task, samb in assign_list.items():
                        select_com_node_list = []   # 该列表用于存储同样任务，不同智能体组合的节点
                        select_t = []   # 该列表用于存储这些节点的makespan
                        for com in samb:
                            new_node = copy.deepcopy(new_init_node)
                            for agent_i in com:
                                new_node[agent_i].append((self.task_data[to_assig_task], sub_task))
                                select_com_node_list.append(new_node)
                        for node in select_com_node_list:   # 评估不同智能体组合的指标水平
                            t, _ = self.opt_for_partial_assigment(new_node, new_assiged_task)
                            select_t.append(t)
                        selected_node = select_com_node_list[select_t.index(min(select_t))]     # 选makespan最小的node(智能体组合)
                    if len(new_assiged_task) == len(self.task_data):    # 判断是不是所有子任务都分配完了
                        sample_list.append(selected_node)
                        un_found = un_found - 1
                    else:
                        sequence.append((new_assiged_task, selected_node))
                        t = select_t[select_t.index(min(select_t))]
                        # t, _ = self.opt_for_partial_assigment(selected_node, new_assiged_task)
                        # print('time',t)
                        t2 = 0
                        for i in new_assiged_task:
                            ...
                            for n in self.task_type[self.task_data[i][1]][1].values():
                                score = score + n
                            t2 = t2 + self.task_type[self.task_data[i][1]][0] * score/10
                            # t2=t2+self.task_type[self.task_data[i][1]][0]
                        if t <= 0.1:
                            tstar = 1
                        else:
    ...
    def get_lower_bound_with_i_j(self, node, assigned_tasks):
        ...
        begin_time_list = []    # 获取每个智能体执行完node中任务后的时刻
        for agent in node:
            if len(agent) == 0:
        ...
        for i in unassigned_tasks:
            task_dic[i] = z
            z = z + 1
        agent_pose = []     # 获取智能体执行完node中任务后所在位置
        ...
        for task_j in unassigned_tasks:
            ...
            for agent in self.agent_data:
                ...
                time_table.append(time)     # 计算出每个智能体到任务目标区域的时间
            for task in unassigned_tasks:
                if not task == task_j:
                    time = self.get_distance(self.task_data[task_j][2], self.task_data[task][2]) / 10
                    time_table.append(time)     # 计算出该任务与其他任务区域之间的转移时间
        ...
        if len(np.shape(b_time)) > 2:
            s = 1
        constrain_begin_time = [M_time @ x_i_j + T_time @ t_i >= b_time]    # 任务结果时间 - duration - 区域转移时间 >= 智能体可用时间
    ...
    def exten_child_nodes(self, node, assign_task):     # 根据R-poset Graph以及动作所需智能体数量扩展子节点
        ...
        for task in to_assig_set:   # task: (0, 'scout', 'b01')
            ...
            pot_agent_list = []
            req_score = 0
            for sub_task, score in self.task_type[task[1]][1].items():    # sub_task和num分别指action和它所需的智能体数量
                list1 = []      # 存储候选智能体的id
                req_score = req_score + score
                for agent_i in self.agent_data:     # self.agent_data:[(0/id, 'AUAVbase', 'AUAV'), (1, 'AUAVbase', 'AUAV'),...]
                    if sub_task in self.agent_type[agent_i[2]]['serve'].keys():    # self.agent_type:{‘AUAV’: {'serve': {'attack':10}, 'velocity': 20},...}
                        list1.append(agent_i[0])
                        
                agent_list = []
                for i in range(3):  # 找50次智能体组合
                    agent_con_i = []
                    con_score = 0
                    while con_score < score:
                        agent_id = random.choice(list1)
                        ...
                        agent_con_i.append(agent_id)
                        con_score = con_score + self.agent_type[self.agent_data[agent_id][2]]['serve'][sub_task]
                    if sorted(agent_con_i) not in agent_list:
                        agent_list.append(sorted(agent_con_i))
                pot_agent_list.append(agent_list)
                # combina = iter_com(list1)      # 生成所有可能的智能体id组合（list1中挑num个出来）
            ...
            for assig_list in pot_agent_list[0]:   # assig_list: (1,2,3,4)
                # t = list(assig_list)
                # for i in assig_list:
                ...
                for agent_id in assig_list:
                    child_node[agent_id].append((task, sub_task))    # ((0, 'scout', 'b01'), 'scout')
    ...
    def opt_for_partial_assigment(self, node, assign_task, i=None):     # 根据节点已分配的任务计算出在满足约束下的最优方案
        ...
        TW = self.time_window
        for i in assign_task:
            action = self.task_data[i][1]
            region = self.task_data[i][2]
            for subtask, tw in TW.items():
                if action in subtask and region in subtask:
                    tw_i = tw
            m = [0 for l in range(len(assign_task))]
            m[assign_task_dic[self.task_data[i][0]]] = 1
            M3.append(m)
            ...
            m[assign_task_dic[self.task_data[i][0]]] = -1
            M3.append(m)
            B3[0].append(-tw_i[0]-self.task_type[self.task_data[i][1]][0])  # ei - di >= esti
