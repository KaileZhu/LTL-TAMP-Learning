...
class Branch_And_Bound(object):
    def __init__(self,poset,task_data,input_data, motionmap=None):
        ...
        self.motionmap = motionmap
        self.Astar_table = list()
        self.get_horizon()
        self.explored_node_dic = dict()
...
#=======motion_constrain
    ...
    def get_distance(self, i, j):
        if self.motionmap == None:
            pos1=self.position[i][0]-self.position[j][0]
            pos2=self.position[i][1]-self.position[j][1]
            lenth=(pos1**2+pos2**2)**0.5
            return lenth
        else:
            for pos, roi in self.motionmap.regions.items():
                if i == roi['name']: pos_i = pos
                if j == roi['name']: pos_j = pos
            return self.motionmap.costmap[pos_i][pos_j]['len']
