...
from ltl_mas.simulate.astar import AStar
...
class field:
    ...
    def plot_back_ground_with_time(self,agent_type,pose_trajectory,stage_track,time,color_table,pre_name,type='with_word'):
        ...
        for i in range(len(pose_trajectory[0])):
            ...
            if self.agent_type[i]=='UAV':
                ...
                category_colors=plt.get_cmap(color_varide[5])(np.linspace(0.6,1,(time-subtime)))
            ...
            if 'UAV' in agent_type[i]:
                ...
                path_x=trajectory_x[0:time-subtime:25]
                path_y=trajectory_y[0:time-subtime:25]
    ...
    def plan_the_fix_action(self,place):
        ...
        path_UGV_s=[[[center[0]+r*np.sin(np.pi*i/180),center[1]+r*np.cos(np.pi*i/180)] for i in range(80,420,5)]]
    ...
    def set_the_input_msg_for_task_assignment(self):
        ...
        agent_type={'UAV':{'serve':['temp','scan','wash_UAV','back'],'velocity':10.0},
            'UGV_l':{'serve':['wash_UGV_l','repair_UGV_l','back','fix_UGV_l'],'velocity':2.0},
                   'UGV_s':{'serve':['sweep','mow','repair_UGV_s','back','fix_UGV_s'],'velocity':4.0} }
