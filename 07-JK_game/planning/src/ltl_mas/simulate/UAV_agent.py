from ltl_mas.simulate.arastar import AraStar
from ltl_mas.simulate.astar import AStar
from data import input_data
from ltl_mas.simulate.field_background import photovoltaic_Env
import copy
import random
    ...
    def execute_error(self,situation=1):
        ...
        if situation==2:
            if self.agent_ID in [3]:
                if self.current_time>300:
                    self.error_time=0
            if self.agent_ID in [8,0]:
                if self.current_time>720:
                    self.error_time=0
