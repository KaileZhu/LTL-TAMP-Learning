...
# Function: VIKOR
def vikor_method(dataset, weights, criterion_type, strategy_coefficient=0.5, graph=False):
    X = np.copy(dataset)
    w = np.copy(weights)
    ...
    if (condition_1 == True and condition_2 == False):
        solution = np.copy(flow_q[0:2, :])  # 仅存储q前两个方案
    # 不满足条件1
    ...
    for i in range(0, solution.shape[0]):
        print(str(i + 1), str(round(solution[i, 0], 2)))
    if (graph == True):  # 默认不画图
        ranking(solution)
