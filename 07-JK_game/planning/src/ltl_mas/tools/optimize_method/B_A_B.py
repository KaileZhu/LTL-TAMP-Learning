...
class Branch_And_Bound(object):
    def __init__(self,poset,task_data,input_data):
        '''
        这部分为合同网的处理算法，目的将任务序列在给定的时间内，合理分配给目标智能体，并保持协作的功能。
        poset 为输入的偏序集要求
        task_date 为任务信息
        input_date 为环境信息
        '''
        ...
        self.sub_task_type=input_data.task_data_cost
        self.agent_type=input_data.agent_type
        self.position_center=input_data.position_center
        self.Astar_table=[]
        self.get_horizon()
        self.explored_node_dic={}
        self.generate_poset_graph()
        self.initial_poset_state()
    ...
    def Begin_branch_search(self,time_limit,up_bound_method=1,low_bound_method=1,search_method='DFS'):
        start = time.time()
        self.generate_time_budget()
        ...
        self.count_round=1
        up_bound,solution=self.get_upper_bound(root_node,assigned_tasks,'greedy')
        ...
        #child_nodes=self.branching_routine(search_method)
            if tuple(task) in self.search_node_list.keys():
                if node not in self.search_node_list[tuple(task)]:
                    ...
                    child_nodes=self.exten_child_nodes(node,task)
                    print('生成投标方案，投标方案当前分配的任务为',task)
                    self.branch_tree.extend(child_nodes)
                    self.astar_list_in_tree.extend(self.estimate_Astar_value(child_nodes, node_up_bound))
    ...
    def initial_poset_state(self):
        self.task_situation={'finsihed':[],'executing':[],'feasible':[],'forbidden':[]}
        #judge the four property above
        ...
        #here unassig_task is maight be feasible or forbidden by some neq relation
        self.task_situation['feasible']=un_assig_task
    ...
    def estimate_Astar_value(self,node_list,upper_bound):
        ...
        for node,task in node_list:
            ...
            execute_time=0
            for i in task:
                execute_num = self.sub_task_type[i][1]
                t2 = t2 + self.sub_task_type[i][0] * execute_num+self.subtask_value[i]['priority']/5
            #考虑优化偏好
            #优化偏好有 最短时间  最高效费比 最高成功率
            active_agent=1
            for agent in node:
                if len(agent)>0:
                    active_agent=active_agent+1
            if self.prefer_type=='最短时间':
                Astar_list.append(t2/upper_bound/len(self.agent_data))
            if self.prefer_type=='最高效费比':
                Astar_list.append(t2/upper_bound/len(self.agent_data)+4/active_agent)
            if self.prefer_type=='最高成功率':
                Astar_list.append(t2/upper_bound/len(self.agent_data)+(4-4/active_agent))
    ...
    def found_solution_greedy(self,init_node,assigned_tasks,repeat_number=1):
        ...
        for i in assigned_tasks:
            execute_num = self.sub_task_type[self.task_data[i][0]][1]
            #for n in self.task_type[self.task_data[i][1]][1].values():
            #    execute_num = execute_num + n
            t2 = t2 + self.sub_task_type[self.task_data[i][0]][0] * execute_num
        ...
        while sequence!=[] and un_found>0:
            ...
            for to_assig_task in un_assig_task:
                ...
                sub_task_list=self.sub_task_type[to_assig_task][1]
                sub_task=self.task_data[to_assig_task][2]
                feasible={}
                #for sub_task,num in sub_task_list.items():
                list1=[]
                z=0
                for agent_i,detail in self.agent_data.items():
                    if sub_task in self.agent_type[detail['type']]['serve']:
                        #检测资源的限制
                        if sub_task == 'attack':
                            if len(init_node[z])<agent_resource[detail['type']]:
                                list1.append(z)
                        else:
                            list1.append(z)
                    z=z+1
                feasible[sub_task]=(list1,sub_task_list)
                if len(list1)<sub_task_list:
                    1
                    #print(sub_task,list1,sub_task_list)
            #=====================get task distribution
                for z in range(repeat_number):
                    ...
                    while 1:
                        assign_list={}
                        check_list=[]
                        for sub_task,(list1,num) in feasible.items():
                            if len(list1)< num:
                                print('资源限制，任务需求暂时无法满足，自动适应当前情况',sub_task,list1,num)
                                assign_list[sub_task] =list1
                            else:
                                assign_list[sub_task]=random.sample(list1,num)
                            check_list.extend(assign_list[sub_task])
                            repet_Num=Counter(check_list)
                        #这里加入资源的限制
    ...
    def found_solution_greedy2(self,init_node,assigned_tasks):
        ...
        while sequence!=[] and un_found>0:
            ...
            for to_assig_task in un_assig_task:
                new_assiged_task=assiged_task |{to_assig_task}
                #print(self.task_type.keys())
                #need how many agent to join the task
                sub_task_list=self.sub_task_type[self.task_data[to_assig_task][0]][1]
                feasible={}
                list1=[]
                for agent_i, detail in self.agent_data.items():
                    if self.task_data[to_assig_task][2] in self.agent_type[detail['type']]['serve']:
                        list1.append(agent_i)
                feasible[self.task_data[to_assig_task][2]]=(list1,sub_task_list)
                #for sub_task,num in sub_task_list.items():
                #    list=[]
                #    for agent_i in self.agent_data:
                #        if sub_task in self.agent_type[agent_i[2]]['serve']:
            ...
            #=====================get task distribution
                ...
                while 1:
                    ...
                    for sub_task,(list1,num) in feasible.items():
                        if len(list1)< num:
                            #print(len(list1))
                            #print(num)
                            raise  ValueError('没有足够的智能体提供对应功能')
                        assign_list[sub_task]=random.sample(list1,num)
                        check_list.extend(assign_list[sub_task])
                        repet_Num=Counter(check_list)
                    if len(repet_Num)==len(check_list):
                ...
                for sub_task,samb in assign_list.items():
                    for agent_i in samb:
                        n=list(self.agent_data.keys()).index(agent_i)
                        #print((self.task_data[to_assig_task],sub_task))
                        #print(agent_i)
                        #print(new_node[n])
                        new_node[n].append((self.task_data[to_assig_task],sub_task))
                if len(new_assiged_task)==len(self.task_data):
                    sample_list.append(new_node)
                    un_found=un_found-1
                else:
                    ...
                    t2=0
                    for i in new_assiged_task:
                        t2=t2+self.sub_task_type[self.task_data[i][0]][0]
                    if t<=0.1:
                        tstar=1
                    else:
                        tstar=t2/t/len(self.agent_data)
    ...
    def generate_poset_graph(self):
        ...
        if len(self.poset['<='])>100:
            poset_list=random.sample(self.poset['<='],100)
            self.poset['<=']=poset_list
            for i,j in poset_list:
                poset_graph.add_edge(i, j)
        else:
            for i,j in self.poset['<=']:
                poset_graph.add_edge(i,j)
    ...
    def get_time_table_of_best_solution(self,solution):
        task_time_cost_list=[task[0] for task in self.sub_task_type]
        t,end_time=self.opt_for_partial_assigment(solution,range(len(self.task_data)))
        self.task_time_table=[[i,end_time[i][0]-task_time_cost_list[i],end_time[i][0]] for i in range(len(end_time))]
        print('任务执行时间列表为：')
        for time_table in self.task_time_table:
            print(time_table)
        task_order_list=self.task_time_table.copy()
        task_order_list.sort(reverse=True , key=lambda x:x[1])
        print('任务执行顺序为')
        print([x[0] for x in task_order_list])
    ...
    def get_lower_bound_with_i_add_j(self,node,assigned_tasks):
        ...
        for agent in self.agent_data:
            if len(node[num_i])==0:
                agent_pose.append((self.agent_data[agent]['pose'][0],self.agent_data[agent]['pose'][1]))
            else:
                pos=self.position_center[node[num_i][-1][0][3]]
                agent_pose.append(pos)
            num_i=num_i+1
        agent_number=len(self.agent_data)
        t_i=cp.Variable(shape=1,nonneg=True)
        ...
        for task_j in unassigned_tasks:
            time_table=[]
            agent_number=0
            for agent,detial in self.agent_data.items():
                x1=self.position_center[self.task_data[task_j][3]]
                x2=agent_pose[agent_number]
                time=((x1[0]-x2[0])**2+(x1[1]-x2[1])**2)**0.5/self.agent_type[detial['type']]['velocity']
                time_table.append(time)
                agent_number=agent_number+1
            for task in unassigned_tasks:
                if not task==task_j:
                    pose1=self.position_center[self.task_data[task_j][3]]
                    pose2=self.position_center[self.task_data[task][3]]
                    time=self.get_distance(pose1,pose2)
                    #time=self.get_distance(self.task_data[task_j][2],self.task_data[task][2])/100
                    time_table.append(time)
            new_task_time=self.sub_task_type[self.task_data[task_j][0]][0]+min(time_table)
            new_task_time_set[task_j]=new_task_time
        self.new_task_time_set=new_task_time_set
        #poset constrain
        for i,j in self.poset['<=']:
        ...
        #================provide enough serves for the task j (2)
        for task_j in task_dic.keys():
            task_subs=self.sub_task_type[self.task_data[task_j][0]][1]
            unassigned_task_time=unassigned_task_time+task_subs*self.sub_task_type[self.task_data[task_j][0]][0]
        constrain_t_i=[agent_number*t_i-sum(begin_time_list) -unassigned_task_time >=0]
        ...
        if not total_constrain==[]:
            ...
            prob_1=cp.Problem(obj_1)
        prob_1.solve(solver="ECOS")
        prob_2=cp.Problem(obj_2,constrain_t_i)
        #solver: "GLPK_MI ECOS CVXOPT ECOS_BB"
        prob_2.solve(solver="ECOS")
        value=max(prob_1.value,prob_2.value,max(begin_time_list))
        #print('low bound:',value)
        return value
    ...
    def exten_child_nodes(self,node,assign_task):
        ...
        for task in to_assig_set:
            ...
            sub_task_list=[]
            pot_agent_list=[]
            #for sub_task,num in range(self.sub_task_type[task[0]][1]):
            num=self.sub_task_type[task[0]][1]
            sub_task=task[2]
            list1=[]
            z=0
            for agent_i,detail in self.agent_data.items():
                if sub_task in self.agent_type[detail['type']]['serve']:
                    list1.append(z)
                z=z+1
            #add the limit of source in node extention
            list1=self.consider_source_limited(list1,node,task)
            #combina=iter_com(list1,num)
            combina=self.new_itercom(list1,num)
            #print(list1,num)
            #n=len(list(iter_com(list1,num)))
            n=len(list(self.new_itercom(list1,num)))
            pot_agent_list.append(random.sample(list(combina),min(n,200)))
    ...
    def new_itercom(self,list1,num):
        #生成一系列更为短小的
        limited=200
        count=0
        n=len(list1)
        if num+2 > n:
            return iter_com(list1,num)
        else:
            random.shuffle(list1)
            new_list=random.sample(list1,num+2)
            return iter_com(new_list, num)





    def consider_source_limited(self,list1,node,task):
        #print('here is  the task',task)
        if task[2]=='attack':
            feasible_list=[]
            agent_name=list(self.agent_data.keys())
            for agent in list1:
                agent_type=self.agent_data[agent_name[agent]]['type']

                source_limit=agent_resource[agent_type]
                agent_action=node[agent]
                #source_limit=3
                source_num=0
                for sub_task in agent_action:
                    if sub_task[1]=='attack':
                        source_num=source_num+1
                if source_limit>source_num:
                    feasible_list.append(agent)
    ...
    def opt_for_partial_assigment(self, node, assign_task, i=None):
        ...
        for i,j in self.poset['<=']:
            if self.task_data[i][0] in assign_task and self.task_data[j][0] in assign_task:
                ...
                #    M1.append(m)
                #    B1[0].append(-self.sub_task_type[self.task_data[j][0]][0]+self.sub_task_type[self.task_data[i][0]][0])
                #else:
                    #<
                    m=[0 for l in range(len(assign_task))]
                    m[assign_task_dic[self.task_data[i][0]]]=1
                    m[assign_task_dic[self.task_data[j][0]]]=-1
                    M1.append(m)
                    B1[0].append(-self.sub_task_type[self.task_data[j][0]][0])
        for i,j in self.poset['=']:
            if self.task_data[i][0] in assign_task and self.task_data[j][0] in assign_task:
                #if self.task_type[self.task_data[i][0]][0]>= self.task_type[self.task_data[j][1]][0]:
                if self.sub_task_type[self.task_type[i][0]][0]>=self.sub_task_type[self.sub_task_type[j][1]][0]:
                    changelabel=-1
                else:
                    changelabel=1
                m=[0 for l in range(len(assign_task))]
                ...
                m[assign_task_dic[self.task_data[j][0]]]=changelabel
                M1.append(m)
                B1[0].append( -self.sub_task_type[self.task_data[i][0]][0]*changelabel+self.sub_task_type[self.task_data[j][0]][0]*changelabel)
        for i,j in self.poset['!=']:
            if i in assign_task and j in assign_task:
                if not (i,j) in self.poset['<='] and not (j,i) in self.poset['<=']:
                    ...
                    constrain0=[m @ end_time -self.sub_task_type[self.task_data[i][0]][0] -bool_for_x * self.time_budget+self.time_budget>=0]
                    constrain1=[m @ end_time + self.sub_task_type[self.task_data[j][0]][0] + bool_for_x * self.time_budget >=0]
                    #print(1)
        ...
        M2=[]
        B2=[[]]
        for agent_i in self.agent_data.keys():
            n_of_agent_i=list(self.agent_data.keys()).index(agent_i)
            if len(node[n_of_agent_i])>0:
                m=[0 for i in range(len(assign_task))]
                #print(assign_task_dic)
                #print(node[agent_i][0][0])
                #print(assign_task_dic)
                c=assign_task_dic[node[n_of_agent_i][0][0][0]]
                m[c]=1
                ...
                M2.append(m)
                pose1=self.agent_data[agent_i]['pose']
                pose2=self.position_center[node[n_of_agent_i][0][0][3]]
                dis=self.get_distance(pose1,pose2)
                #print('dis=',dis)
                #print('cost of time',dis/self.agent_type[self.agent_data[agent_i]['type']]['velocity'])
                B2[0].append(dis/self.agent_type[self.agent_data[agent_i]['type']]['velocity']+\
                    self.sub_task_type[node[n_of_agent_i][0][0][0]][0])
            if len(node[n_of_agent_i])>1:
                for task in range(len(node[n_of_agent_i])-1):
                    #-endi+endj>=dis+durj
                    m=[0 for i in range(len(assign_task))]
                    c=assign_task_dic[node[n_of_agent_i][task][0][0]]
                    m[c]=-1
                    c=assign_task_dic[node[n_of_agent_i][task+1][0][0]]
                    m[c]=1
                    pose1=self.position_center[node[n_of_agent_i][task][0][3]]
                    pose2=self.position_center[node[n_of_agent_i][task+1][0][3]]
                    dis=self.get_distance(pose1,pose2)
                    b=dis/self.agent_type[self.agent_data[agent_i]['type']]['velocity']+\
                        self.sub_task_type[node[n_of_agent_i][task+1][0][0]][0]
                    #print('cost of time',b)
                    M2.append(m)
                    B2[0].append((b))
        if M2==[]:
            s=1
                ...
                if  i in assign_task:
                    if  j in assign_task:
                        ...
                        constrain1 = [m @ end_time + self.task_type[self.task_data[j][1]][
                            0] + bool_for_x * self.time_budget >= 0]
                        total_constrain.append(*constrain0)
                        total_constrain.append(*constrain1)
...
#=======motion_constrain
    ...
    def get_distance(self,i,j):
        ...
        lenth=((i[0]-j[0])**2+(i[1]-j[1])**2)**0.5
        return lenth

    def print_answer(self):
        #print('best value is:',self.best_up_bound)
        for i in range(len(self.agent_data)):
            print('智能体',i+1,'分配得到的任务列表为',self.best_solution[i])


    def swarm_menager(self):
        #设置集群分布的关系
        # 返回一个集群的分布
        # 集群设置？根据任务分？
        swam_dic_about_task={}
        ...
        for agent in self.best_solution:
            for task in agent:
                if task[0][0] in swam_dic_about_task.keys():
                    swam_dic_about_task[task[0][0]]=[agent_ID]
                else:
                    swam_dic_about_task[task[0][0]].append(agent_ID)
            agent_ID=agent_ID+1
        #子任务是以集群的形式执行的，每个集群对对应区块进行相应任务的处理
        return swam_dic_about_task

    def judge_the_performance_of_solution(self):
        #输出3个值
        #作战危险性量化指标
        danger_list=[]
        wanhao_list=[]
        ...
        for agent in self.best_solution:
            ...
            if n==0:
                ...
                danger_list.append(1/n)
                wanhao_list.append(1/n/n)
            if n>4:
                lose_list=lose_list+1
            else:
                win_list=win_list+1

        danger_level=sum(danger_list)/len(danger_list)
        wanhao_level=sum(wanhao_list)/len(wanhao_list)
        lose_number=0.1*lose_list/(lose_list+win_list)
        win_number=0.1*win_list/(lose_list+win_list)


        #计算方法，单纯统计敌方的战斗力数量， 统计我方的指标

        #集群完好度量化指标
        #敌方战斗力 vs 我方战斗力 求比例的平方项 然后 归一化 最后求百分比
        ...
        base_level=0.9
        shenglv=(base_level-lose_list,base_level+win_list)
        level_set={'wenxian':wanhao_level,'wanhao':wanhao_level,'shenglv':shenglv}
        return level_set