...
red_type_3 = ['RC', 'RUSV', 'RSUAV', 'RAUAV', 'RM', 'RAD']

red_RC_3 = ['Red.RedForce_RC#0']
# red_RUSV = ['Red.RedForce_RC#0_RUSV#0', 'Red.RedForce_RC#0_RUSV#1', 'Red.RedForce_RC#0_RUSV#2', 'Red.RedForce_RC#0_RUSV#3',
...
red_RSUAV_3 = ['Red.RedForce_RC#0_RSUAV#0', 'Red.RedForce_RC#0_RSUAV#1', 'Red.RedForce_RC#0_RSUAV#2', 'Red.RedForce_RC#0_RSUAV#3',
             'Red.RedForce_RC#0_RSUAV#4']
red_RAUAV_3 = ['Red.RedForce_RC#0_RAUAV#0', 'Red.RedForce_RC#0_RAUAV#1', 'Red.RedForce_RC#0_RAUAV#2', 'Red.RedForce_RC#0_RAUAV#3',
...
red_RM_3 = ['Red.RedForce_RC#0_RM#0', 'Red.RedForce_RC#0_RM#1', 'Red.RedForce_RC#0_RM#2', 'Red.RedForce_RC#0_RM#3',
...
red_RAD_3 = ['Red.RedForce_RC#0_RAD#0', 'Red.RedForce_RC#0_RAD#1', 'Red.RedForce_RC#0_RAD#2']

blue_type_3 = ['BC', 'BUSV', 'BSUAV', 'BAUAV', 'BM', 'BAD']

blue_BC_3 = ['Blue.BlueForce_BC#0']
# blue_BUSV = ['Blue.BlueForce_BC#0_BUSV#0', 'Blue.BlueForce_BC#0_BUSV#1', 'Blue.BlueForce_BC#0_BUSV#2', 'Blue.BlueForce_BC#0_BUSV#3',
#              'Blue.BlueForce_BC#0_BUSV#4']
blue_BSUAV_3 = ['Blue.BlueForce_BC#0_BSUAV#0', 'Blue.BlueForce_BC#0_BSUAV#1', 'Blue.BlueForce_BC#0_BSUAV#2', 'Blue.BlueForce_BC#0_BSUAV#3',
              'Blue.BlueForce_BC#0_BSUAV#4']
blue_BAUAV_3 = ['Blue.BlueForce_BC#0_BAUAV#0']
blue_BM_3 = ['Blue.BlueForce_BC#0_BM#0', 'Blue.BlueForce_BC#0_BM#1', 'Blue.BlueForce_BC#0_BM#2', 'Blue.BlueForce_BC#0_BM#3',
...
blue_BAD_3 = ['Blue.BlueForce_BC#0_BAD#0', 'Blue.BlueForce_BC#0_BAD#1', 'Blue.BlueForce_BC#0_BAD#2']

blue_location_3 = [[39, 47.5], [40, 48], [40, 49], [41, 48.5], [41, 47.8]]
red_location_3 = [[38, 48], [39, 48.5], [39, 49], [38, 49]]
red_location_for_USV_3 = [[121, 24], [120, 23.5]]


red_agent_num_3 = 39
blue_agent_num_3 = 30

def compute_acc_reward_3(obs):
    index = 0
    blue_life = [-1 for _ in range(blue_agent_num_3)]
    for id, entities in obs['blueSituation'].items():
        for entity in entities:
            # if entity['name'] in blue_BC_3:
            #     blue_life[index] = 100
            #     index += 1
            #     continue
            if entity['life'] <= 0:
    ...
    index = 0
    red_life = [1 for _ in range(red_agent_num_3)]
    ...
    for i in range(blue_agent_num_3):
        if blue_life[i] > 0:
            blue_survive_reward = blue_survive_reward + blue_weight[i]

    for i in range(red_agent_num_3):
        if red_life[i] > 0:
...
def get_red_staus_3(obs):
    unit_staus = [-1 for _ in range(red_agent_num_3)]
    index = 0
    for red_id, red_entities in obs['redSituation'].items():
        for red_entity in red_entities:
            if index >= 16 and index < 26:
                index += 1
                continue
    ...
    red_staus['red_damage_rate'] = unit_staus.count(2) / 19
    red_staus['red_live_rate'] = unit_staus.count(1) / 19
    red_staus['red_destroy_rate'] = unit_staus.count(0) / 19

    return red_staus


def get_blue_staus_3(obs):
    unit_staus = [-1 for _ in range(blue_agent_num_3)]
    index = 0
    for blue_id, blue_entities in obs['blueSituation'].items():
        for blue_entity in blue_entities:
            if index >= 7 and index < 27:
                index += 1
                continue
            # if blue_entity['name'] in blue_BC_3:
            #     unit_staus[index] = 1
            #     index += 1
            #     continue
    ...
    blue_staus['blue_damage_rate'] = unit_staus.count(2) / 10
    blue_staus['blue_live_rate'] = unit_staus.count(1) / 10
    blue_staus['blue_destroy_rate'] = unit_staus.count(0) / 10
...
def get_strength_3(obs):
    index = 0
    blue_mi_strength = 0
    for blue_id, blue_entities in obs['blueSituation'].items():
        for blue_entity in blue_entities:
            # if blue_entity['name'] in blue_BC_3:
            #     blue_mi_strength += blue_strength_3[index] * 100 / blue_unit_life_2[index]
            #     index += 1
            #     continue
            if blue_entity['life'] > 0:
                blue_mi_strength += blue_strength_3[index] * blue_entity['life'] / blue_unit_life_2[index]
            index += 1
    ...
    for red_id, red_entities in obs['redSituation'].items():
        for red_entity in red_entities:
            if red_entity['life'] > 0:
                red_mi_strength += red_strength_3[index] * red_entity['life'] / red_unit_life_2[index]
            index += 1
...
def get_value_3(obs):
    ...
    for blue_id, blue_entities in obs['blueSituation'].items():
        for blue_entity in blue_entities:
            # if blue_entity['name'] in blue_BC_3:
            #     blue_mi_value += blue_value_3[index]
            if blue_entity['life'] > 0:
                blue_mi_value += blue_value_3[index]
            index += 1
    ...
    for red_id, red_entities in obs['redSituation'].items():
        for red_entity in red_entities:
            if red_entity['life'] > 0:
                red_mi_value += red_value_3[index]
            index += 1
    return red_mi_value, blue_mi_value


def get_detect_3(obs, detect_list):
    index = 0
    for blue_id, blue_entities in obs['blueSituation'].items():
        for blue_entity in blue_entities:
            if detect_list[index] == 1:
                ...
                for red_id, red_entities in obs['redSituation'].items():
                    for red_entity in red_entities:
                        if red_entity["name"] in red_RSUAV_3 or red_entity["name"] in red_RAUAV_3:
...
class Logger3:
    def __init__(self):
        self.detect_list = [0 for i in range(blue_agent_num_3)]
    ...
    def update(self, situationDataDict, reward_list):
        ...
        red_staus = get_red_staus_3(situationDataDict)
        ...
        blue_staus = get_blue_staus_3(situationDataDict)
        ...
        detect_ratio, self.detect_list = get_detect_3(situationDataDict, self.detect_list)
        ...
        red_mi_strength, blue_mi_strength = get_strength_3(situationDataDict)
        ...
        red_mi_value, blue_mi_value = get_value_3(situationDataDict)
        ...
        reward = compute_acc_reward_3(situationDataDict)
        ...
        for red_id, red_entities in situationDataDict['redSituation'].items():
            for red_entity in red_entities:
                ...
                if red_entity["name"] in red_RSUAV_3:
                    unit_lat = red_entity['latitude']
                    unit_lon = red_entity['longitude']
                ...
                if red_entity["name"] in red_RAUAV_3:
                    unit_lat = red_entity['latitude']
                    unit_lon = red_entity['longitude']
        ...
        for blue_id, blue_entities in situationDataDict['blueSituation'].items():
            for blue_entity in blue_entities:
                ...
                #     file.write(str(unit_lon) + ',\n')
                #     file.close()
                if blue_entity["name"] in blue_BSUAV_3:
                    unit_lat = blue_entity['latitude']
                    unit_lon = blue_entity['longitude']
                    ...
                    file.write(str(unit_lon) + ',\n')
                    file.close()
                if blue_entity["name"] in blue_BAUAV_3:
                    unit_lat = blue_entity['latitude']
                    unit_lon = blue_entity['longitude']
        ...
        plt.ylim(ymin=47, ymax=50)
        plt.xlim(xmin=36, xmax=42)
...
def blue_actions_3(situation, command):
    redSituation = situation['redSituation']
    blueSituation = situation['blueSituation']
    for blue_id, blue_entities in blueSituation.items():
        if blue_id == blue_type_3[1]:
            continue
        elif blue_id == blue_type_3[2]:
            for blue_entity in blue_entities:
                blue_name = blue_entity['name']

                blue_action = {}
                blue_action['agentname'] = blue_name
                blue_action['longitude'] = blue_location_3[int(blue_name[-1])][0] + random.random() * 0.25
                blue_action['latitude'] = blue_location_3[int(blue_name[-1])][1] + random.random() * 0.25
                blue_action['altitude'] = 0
                command['moveactions'].append(blue_action)
        elif blue_id == blue_type_3[3]:
            for blue_entity in blue_entities:
                blue_name = blue_entity['name']
        ...
        elif blue_id == blue_type_3[4]:
            red_RAD_location = []
            for red_id, red_entities in redSituation.items():
                if red_id == red_type_3[-1]:
                    for red_entity in red_entities:
            ...
            for blue_entity in blue_entities:
                ...
                if m < 3:
                    ...
                    blue_action1 = {}
                    blue_action1['agentname'] = blue_name
                    blue_action1['targetname'] = [red_RAD_3[m]]
                    command['fireactions'].append(blue_action1)

    return command


def red_actions_3(situation, command):
    redSituation = situation['redSituation']
    blueSituation = situation['blueSituation']
    for red_id, red_entities in redSituation.items():
        if red_id == red_type_3[2]:
            for red_entity in red_entities:
                ...
                if situation['step'] < 10:
                    ...
                    red_action['longitude'] = red_location_3[int(red_name[-1]) % 2][0] + random.random() * 0.25
                    red_action['latitude'] = red_location_3[int(red_name[-1]) % 2][1] + random.random() * 0.25
        ...
        elif red_id == red_type_3[1]:
            continue
        elif red_id == red_type_3[3]:
            for red_entity in red_entities:
                red_name = red_entity['name']
                red_lon = red_entity['longitude']
                red_lat = red_entity['latitude']
                    ...
                    for blue_id, blue_entities in blueSituation.items():
                        if blue_id == blue_type_3[1]:
                            ...
                            if blue_targetname is not None:
                                ...
                                target_range = 10000
                                for blue_id, blue_entities in blueSituation.items():
                                    if blue_id != blue_type_3[0] and blue_id != blue_type_3[4]:
                                        for blue_entity in blue_entities:
                                            if blue_entity['life'] > 0:
                                                if target_range > geodesic(
                                                        (blue_entity['latitude'], blue_entity['longitude']),
