...
\While{$time<t_0$}{
	...
	\If{$W \notin \mathcal{L}_\varphi$\label{alg1:inlp}}{
                Set~$P=(\Omega,\, \preceq_{\varphi},\, \neq_{\varphi})$,
                and $\preceq_{\varphi}=\neq_{\varphi}=\mathbb{F}$;\\
		Set~$L(P)=Que=\{W\}$ and~$I_1=I_2=\emptyset$;\label{alg-line:initial}\\
		\tcc{Reduce partial relations}
		\While{$|Que|>0$\label{alg1:begin_que}}{
			$W \leftarrow Que.pop()$;\\
			\For{$i=1,2,\cdots,|\rho|$}{
				$\omega_1=W[i]$, $\omega_2=W[i+1]$;\\
				$W' \leftarrow $ Switching~$\omega_1$
                                and~$\omega_2$ within~$W$\label{alg1:switch};\\
				...
				}{Add $(\omega_1,\,\omega_2)$ to $I_2$;\label{alg1:end_que}}
		}}
                Remove $\{I_1\,\backslash\, I_2\}$
                from~$\preceq_{\varphi} $;\label{alg-line:remove}\\
		%Remove $W$ conflict $\preceq_\varphi$ in $L(P)$;\label{alg1:remove W}\\
		\For{$(\omega_1,\,\omega_2) \in \neq_{\varphi}$\label{alg-line:calculate_neq}}{
	        	$W' \leftarrow $ Replace~$\omega_1,\,\omega_2$ in~$W$
                        by~$\omega_1\cup \omega_2$;\label{alg1:word-neq}\\
			\If{$W'$ is accepting\label{alg1:waccepting-neq}}{
                        Remove~$(\omega_1,\,\omega_2)$
                        from~$\neq_{\varphi}$;\label{alg-line:remove_neq}\\}
		}
	%\todoliu{Remove word conflict $P$ in $L(P)$}\\
