...
import numpy as np
from src.ltl_mas.simulate.UAV_agent import  UAV_agent
import random
import copy
from src.ltl_mas.simulate.plotting import Plotting
import cvxpy as cp
from src.ltl_mas.tools.optimize_method.B_A_B_for_suffix import Branch_And_Bound
...
class Agent_swarm(object):
    ...
    def set_difference_init_pose(self):
        ...
        i=0
        for agent in self.task_list:
            if len(agent)>0:
                x,y=self.background.input_data.position[agent[0][0][2]]
                self.agent_swarm[i].init_pose[0] = x
                self.agent_swarm[i].init_pose[1] = y
                self.agent_swarm[i].current_pose[0] = x
                self.agent_swarm[i].current_pose[1] = y
            i=i+1
    ...
    def add_barrier_to_each_agent(self):
        for agent in self.agent_swarm:
            ...
            if agent.agent_type=='UGV_l':
                ...
                for name,node_list in self.field.node_set_for_panel.items():
                    agent.barrier.extend(node_list)
            if agent.agent_type=='UAV':
                agent.barrier=[]
            agent.barrier.extend(self.field.node_set_for_panel['p24'])
    ...
    def begin_run(self,time_buget,time_step,online_update=0):
        ...
        while self.global_time<time_buget:
            ...
            if self.global_time>900:
                s=1
            if (0, 'repairp3', 'p3') in new_task_list:
                s=1
...
# threshold method
    ...
    def rebuild_task_table(self,finish_task,pose_list,executing_task,type='B&B',situation='do the whole'):
        ...
        if type=='B&B':
            if self.global_time>400:
                ...
                s=1
                new_node=[[((9, 'scanp34', 'p34'), 'scan')],
 [((2, 'scanp3', 'p3'), 'scan')],
 [((9, 'scanp34', 'p34'), 'scan')],
 [],
 [((2, 'scanp3', 'p3'), 'scan'), ((9, 'scanp34', 'p34'), 'scan')],
 [((2, 'scanp3', 'p3'), 'scan'), ((8, 'washp34', 'p34'), 'wash')],
 [],
 [((1, 'repairp3', 'p3'), 'repair_UGV_l')],
 [((8, 'washp34', 'p34'), 'wash')],
 [((1, 'repairp3', 'p3'), 'repair_UGV_s'),
  ((5, 'sweepp21', 'p21'), 'sweep'),
  ((7, 'sweepp27', 'p27'), 'sweep')],
 ...
 [],
 [((7, 'sweepp27', 'p27'), 'sweep')],
 [((5, 'sweepp21', 'p21'), 'sweep'), ((6, 'mowp21', 'p21'), 'mow')]]
        ...
        for agent_ID in range(len(current_node)):
            ...
            for task_ID in range(len(current_node[agent_ID])):
                ...
                if task_ID==0:
                    ...
                    dis_list=[]
                    list1=path_list[task[0][0]][task[1]]
                    if not len(list1)==1:
                    ...
                    if isinstance(path_list[task[0][0]], list):
                        swarm_path[agent_ID].append(path_list[task[0][0]].pop())
                    else:
                        swarm_path[agent_ID].append(path_list[task[0][0]][task[1]].pop())
