...
from src.ltl_mas.tools.boolean_formulas.parser import parse as parse
import copy
import numpy as np
...
class Buchi_poset_builder(object):
    ...
    def __init__(self,task):
        ...
        self.found_action_list_suffix = []
        self._new_buchi2=Buchi(task)
    ...
    def main_fun_to_get_poset(self,time_budget):
        ...
        #self.generate_poset_suffix_anytime(self.new_buchi,time_budget/20)
        ...
        print(1)
        self.essential_time=time.time()-begin_time
    ...
    def main_fun_to_get_path(self):
        ...
        #self.delete_the_edges()
        #self.pruning_step_time=time.time()
        self.generate_path(self.buchi)
    def use_essential_for_act_map(self):
        path_action=[]
        self.poset_action_list=[]
            ...
            for i in range(len(path_action_list[0])):
                for j in range(len(path_action_list[0])):
                        ...
                        if not checker.check(gama0):
                            if not tuple((j,i)) in poset['!=']:
                                poset['!='].add(tuple((i, j)))
            new_action_map = []
            poset['action_map'] = path_action_list[0]
            self.poset_action_list.append(len(path_action_list[0]))
    ...
    def poset_language_shorter(self):
        ...
        self.evaluater_sorter=sorted(range(len(self.language_list)),key=lambda k:evaluater[k])
        self.poset_list=[self.poset_list[i] for i in self.evaluater_sorter]
        #self.task_data_list=[self.task_data_list[i] for i in self.evaluater_sorter]

    def generate_poset_suffix_anytime(self,graph,time_budget):
        # continue the DFS
        self.feasible_edges_list_suffix=[]
        self.unfeasible_edges_list_suffix=[]
        start=self.buchi.graph['accept'][0]
        ends=self.buchi.graph['accept']
        self._prefix_path=[]
        path = []
        paths = []
        self.poset_list_suffix=[]
        queue=[]
        for node in self.new_buchi.successors(self.buchi.graph['accept'][0]):
            queue.append((node,path))
        #queue = [(start, path)]
        start_time=time.time()
        self.poset_start_time=time.time()
        self.poset_start_time_list_suffix=[]
        search_list=[]
        self.language_list_suffix=[]
        while queue and time.time()-start_time<time_budget:
            ...
            if start in ends:
                ...
                if path not in paths:
                    ...
                    if act_list in self.found_action_list_suffix:
                        paths.extend(poset_language_state)
                        continue
                    poset,poset_potential_language=self.find_poset_due_to_one_path_suffix(graph,path)
                    ...
                    self.poset_list_suffix.append(poset)
                    start_1=time.time()
                    self.get_self_loop_list_speed_up(graph,poset_language)
                    end_self=time.time()
                    self.self_loop_time.append(end_self-start_1)
                    self.language_list_suffix.append(-len(poset_language))
                    self.poset_start_time_list_suffix.append(end-start_time)
                    paths.extend(poset_language_state)
        ...
        while queue and time.time()-start_time<time_budget:
            ...
            if start in ends:
                ...
                if path not in paths:
                    ...
                    self.poset_calculate_time.append(time.time())
                    #poset,poset_potential_language=self.find_poset_due_to_essential(graph,path)
                    poset_language_state,poset_language=self.general_poset_language(graph,poset,poset_potential_language,path)
                    #print('poset language is ',len(poset_language))
                    end=time.time()
                    start_1 = time.time()
                    self.get_self_loop_list_speed_up(graph, poset_language_state)
                    end_self = time.time()
                    self.poset_list.append(poset)
                    self.self_loop_time.append(end_self-start_1)
                    self.language_list.append(-len(poset_language))
                    self.poset_start_time_list.append(end-start_time)
                    self.each_poset_time_list.append(end-begin)
                    paths.extend(poset_language_state)
            for node in set(graph.neighbors(start)).difference(path):
                if not node in path:
                    queue.append((node, path))
    ...
    def generate_path(self,graph):
        ...
        while queue :
            ...
            for node in set(graph.neighbors(start)).difference(path):
                if not node in path:
                    queue.append((node, path))
            if np.mod(round,1000)==0:
                print(round)
        print('found path ',round)
            #print('queue', queue)

    def find_poset_due_to_one_path(self,graph,path):
        action_list=self.change_state_path_into_edge(graph,path)
        self_loop_list=self.get_self_loop_list(graph,path)
        poset={'||':set(),'<=':set(),'<':set(),'!=':set(),'=':set(),'action_map':action_list,'self_loop':self_loop_list}
        #'parallel':(a,b) a||b
    ...
    def get_self_loop_list(self,graph,path):
        self_loop_list=[]
        for node in path:
            if graph.has_edge(node,node):
                self_loop_list.append([graph[node][node]['guard_formula']])
            else:
                self_loop_list.append([' '])
        return self_loop_list

    def get_self_loop_list_speed_up(self,graph,poset_language):
        history_dic={}
        graph=self._new_buchi2
        n=poset_language[0]
        i=0
        for node in n:
            history_dic[i]=[]
            i=i+1
        for path in poset_language:
            self_loop_list=[]
            i=0
            for node in path:
                if graph.has_edge(node,node):
                    s=graph[node][node]['guard_formula']
                    if not s in  history_dic[i]:
                        history_dic[i].append(s)
                i=i+1
        return self_loop_list

    def find_poset_due_to_one_path_suffix(self,graph,path):
        graph=self.buchi
        self_loop_list = self.get_self_loop_list(graph, path)
        action_list=self.change_state_path_into_edge(graph,path)
        poset={'||':set(),'<=':set(),'<':set(),'!=':set(),'=':set(),'action_map':action_list,'self-loop':self_loop_list}
        #'parallel':(a,b) a||b
        # 'stirt less-than': (a,b)  a<b
        #'less-than': (a,b)  a<=b
        # 'not equal': (a,b)  a\= b
            ...
            for i in np.arange(len(base_action_map)-1):
                if not (base_action_map[i][0],base_action_map[i+1][0]) in poset['<=']:
                    ...
                    if new_list_map_1 in explored_word:
                        ...
                        label1=self.check_if_action_feasible_suffix(graph,new_action,path[0])
                    if action_list[new_list_map_1[i][0]]==action_list[new_list_map_1[i+1][0]]:
                        label1=0
                    if not label1:
                        if base_action_map[i][0]<base_action_map[i+1][0]:
            ...
            for pre_state in pre_state_set:
                for suf_state in graph.successors(pre_state):
                    if graph[pre_state][suf_state]['guard_formula']==act:
                        ...
                        currect_label=1
                    #else:
                    #    print(graph[pre_state][suf_state]['guard_formula'],'vs',act)
    ...
    def check_if_action_feasible_suffix(self,graph,new_action,begin_state):
        states_list=[begin_state]
        if new_action in self.feasible_edges_list:
            return 1
        if new_action in self.unfeasible_edges_list:
    ...
    def remove_pue_negative_edges(self):
        ...
        while not to_remove_edge_list==[]:
            ...
            if self.new_buchi.has_edge(node2,node1):
                ...
                if not self.new_buchi[node2][node1]['guard_formula']=='(1)':
                    continue
            if node2=='T0_init':
    ...
    def poset_list_reader(self):
        self.task_data_list=[]
        for poset in self.poset_list:
            task_map=poset['action_map']
            task_data=[]
            for i in range(len(task_map)):
                num_left=task_map[i].find('(')
                num_right = task_map[i].find(')')
                num=task_map[i].find('_')
                task_name=task_map[i][num_left+1:num]
                task_place=task_map[i][num+1:num_right]
        ...
        for poset_n in range(len(self.poset_list)):
            ...
            if not double_label:
                ...
                for key,sub_dict in poset.items():
                    if (not key =='action_map') and (not key =='self_loop'):
                        new_poset[key]=set()
