...
from src.ltl_mas.tools.promela import Parser

def run_ltl2ba(formula):
    script_dir = dirname(abspath(__file__))
    # ltl2ba = join(script_dir, "ltl2ba/ltl2ba")
    # ltl2ba = join(script_dir, "ltl2ba/ltl2ba.exe")
    ltl2ba = join(script_dir, "ltl2ba.exe")
    #print(ltl2ba)
    raw_output = check_output([ltl2ba, "-f", "%s" % formula])#if here comes the error, try to imput make in the terminal ltl2ba
