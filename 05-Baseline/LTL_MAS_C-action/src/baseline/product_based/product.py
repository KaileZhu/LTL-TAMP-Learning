...
from src.baseline.product_based.ts import Fts, MotionFts
from src.baseline.product_based.action import LocalActionModel
...
class ProdFts(Fts):
    ...
    def _add_prod_node(self, prod_node):
        ...
        self.add_node(prod_node, label=full_labels)
    ...
    def _compute_full_labels(self, prod_node):
        ...
        for k, k_ts_node in enumerate(prod_node):
            k_labels = self.all_fts[k].nodes[k_ts_node]['label']
