# -*- coding: utf-8 -*-
from src.baseline.product_based.product import ProdFts
from src.baseline.product_based.buchi import Buchi
from src.baseline.product_based.product import ProdTsBuchi
from src.baseline.product_based.utils import add_labels_of_macro_action
import collections
...
class MultiAgentSystem(object):
    ...
    def compute_team_product_ts(self, only_agents=None):
        ...
        #self.add_initial_place()

    def add_initial_place(self):
        """
        add initial
        """
        initial_list=[agent.motact_model.graph['initial'] for agent in self.agents]
        initial_node=tuple(initial_list)
        self._product_ts.graph['initial']=initial_node
