...
#===========================================
def dijkstra_plan_networkX(product, beta=10,Multy=None):
	...
	for prod_target in product.graph['accept']:
                ...
                if prod_target in product.predecessors(prod_target):
                        #print('ueueu',prod_target)
                        loop[prod_target] = (product.edges[prod_target,prod_target]["cost"], [prod_target, prod_target])#here is correct
                        #print(loop[prod_target] )
                        continue
                else:
                        cycle = {}
                        #print('else')
                        ...
                        for target_pred in product.predecessors(prod_target):
                            if target_pred in loop_dist:
                                cycle[target_pred] = product.edges[target_pred,prod_target]["cost"] + loop_dist[target_pred]
                        if cycle:
							#print(cycle.get())
	...
	if runs:
		...
		print('suffix==',suffix)
		run = ProdAut_Run(product, prefix, precost, suffix, sufcost, precost+beta*sufcost,Multy)
		#run= None
		print ('==================')
		print ('Dijkstra_plan_networkX done within ',time.time()-start,':  ',precost,' , sufcost', sufcost)
		return run, time.time()-start,prefix,suffix
		#print '\n==================\n'
	print ('==================')
...
def dijkstra_plan_optimal(product, beta=10, start_set=None):
	...
	if runs:
		...
		#print '\n==================\n'
		print ('optimal_dijkstra_olf done within ',time.time()-start,': precost ',precost,', sufcost',sufcost)
...
def dijkstra_plan_bounded(product, time_limit=3, beta=10):
	...
	accept_set = product.graph['accept']
	init_set = product.graph['initial']
	print ('number of accepting states',len(accept_set))
	print ('number of initial states',len(init_set))
	loop_dict = {}
	for init_prod_node in init_set:
		for (prefix, precost) in dijkstra_targets(product, init_prod_node, accept_set):
			#print 'accept node reached %s' %(str(prefix[-1]))
			...
			if time.time()-start > time_limit:  # time limit has reached
				if runs:
				 	prefix, precost, suffix, sufcost = min(runs.values(), key = lambda p: p[1] + beta*p[3])
				 	run = ProdAut_Run(product, prefix, precost, suffix, sufcost, precost+beta*sufcost)
				 	print ('optimal_dijkstra done within ',time.time()-start,',precost precost,', sufcost, ',sufcost')
				 	return run, time.time()-start
	print ('no accepting run found in optimal planning!')
...
def validate_and_revise_after_ts_change(run, product, sense_info, com_info):
	...
	if changed_regs:
		...
		if new_prefix or new_suffix:
			...
			run.output(product)
			print ('validate_and_revise_after_ts_change done in ',time.time()-star)
		else:
