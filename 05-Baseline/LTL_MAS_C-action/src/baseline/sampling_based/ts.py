...
from src.baseline.sampling_based.utils import compute_distance, find_closest_node
...
class MotionFts(Fts):
    ...
    '''
    def __init__(self, nodes, velocity):
        """
        Args:
        ...
        for (node, labels) in nodes.items():
            self.add_node(node, label=labels)

    def add_edges(self, undirected=True, unit_cost=1.0,
        ...
        for f_node in self.nodes:
            for t_node in self.nodes:
                dist = compute_distance(f_node, t_node, measure='2-norm',
                                        unit=unit_cost, under_flow=under_flow)/self.velocity
