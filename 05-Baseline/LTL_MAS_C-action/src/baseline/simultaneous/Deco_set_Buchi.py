...
class Deco_set_Buchi(object):
    ...
    def main_fun_to_get_decomposible_set(self,time_budget=10):
        self.add_judgement_for_buchi()
        self.feasible_edges_list=[]
        self.unfeasible_edges_list=[]
        start=self.buchi.graph['initial'][0]
        ends=self.buchi.graph['accept']
        self.decomposible_node_set=[]
        path = []
        self.poset_list=[]
        queue = [(start, path)]
        ...
        while queue :
            if count_time<time.time()-start_time:
            ...
            path = path + [start]
            if start in ends:
                self.check_if_decomposable(path)
            for node in set(self.buchi.neighbors(start)).difference(path):
                if not node in path:
                    queue.append((node, path))
        print('find decomposible',len(self.decomposible_node_set))

    def check_if_decomposable(self,path):
        act_list=self.change_state_path_into_edge(self.buchi,path)
        for i in range(len(act_list)):
            if path[i+1] in self.decomposible_node_set:
                continue
            new_act_list1=[act_list[j] for j in range(i)]
            new_act_list2=[act_list[j] for j in range(i,len(act_list))]
            new_act_list2.extend(new_act_list1)
            label=self.check_if_action_feasible(self.buchi,new_act_list2,path[0])
            if label:
                self.decomposible_node_set.append(path[i+1])
                #print(path[i+1])
                self.buchi.nodes[path[i+1]]['virtual_edge']=1

    def add_judgement_for_buchi(self):
        for node in self.buchi.nodes:
            self.buchi.nodes[node]['virtual_edge']=0
    ...
    def generate_poset2(self,action_list_set,time_budget):
        ...
        while unvisited_set and time.time()-begin_time<time_budget:
            if count_time<time.time()-begin_time:
                count_time=count_time+1
                print(time_budget-count_time)
            #print(unvisited_set)
