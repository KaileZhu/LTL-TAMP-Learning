...
def dijkstra_plan_networkX(product, beta=10,weight='cost'):
	...
	for prod_init in product.graph['initial']:
		...
		#print(len(product.graph['accept']))
		#print(product.graph['accept'])
		...
		pose_dic={}
		for node in prefix:
			number_dic[node[0]]=line_dist[node]
			if node[0] not in pose_dic.keys():
				pose_dic[node[0]]=node
		begin_cost_dic={}
		for id, node in pose_dic.items():
			l=prefix.index(node)
			if len(prefix)<=l+1:
				break
			node2=prefix[l+1]
			begin_cost_dic[id]=((node2[1][0][0]-node[1][0][0])**2+(node2[1][0][1]-node[1][0][1])**2)**0.5
		max_time_cost=number_dic[0]+begin_cost_dic[0]
		for key,cost in number_dic.items():
			if key+1 in number_dic.keys():
				max_time_cost=max(max_time_cost,number_dic[key+1]-number_dic[key]+begin_cost_dic[key+1])
	# best combination
	return prefix,max_time_cost
