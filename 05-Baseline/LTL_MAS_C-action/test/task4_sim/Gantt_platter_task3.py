...
def gantt_plotter(node, time, task_data_list ,lenth_n):
    ...
    for agent in node:
        for task in agent:
            ...
            if task[0][0]>len(color)-1:
                plt.barh(i,time[task[0][0]][2]-time[task[0][0]][1],left=time[task[0][0]][1],color=color[task[0][0]-10],hatch='/')
            else:
                plt.barh(i,time[task[0][0]][2]-time[task[0][0]][1],left=time[task[0][0]][1],color=color[task[0][0]])
            print('left=',time[task[0][0]][2])
        i=i+1
    plt.xlim(0,lenth_n*100)
    x_tick=np.linspace(0,lenth_n*100,lenth_n+1)
    ...
    for i in range(len(color)):
        patches.append(mpatches.Patch(color=color[i], label="{:s}".format(labels[i])))
    for i in range(len(color),len(labels)):
        patches.append(mpatches.Patch(color=color[i-10], label="{:s}".format(labels[i]),hatch='/'))
